...
\noindent Curso: Sistemas de Informação\\
...
\ex{2}~Para o conjunto dos números {\bf inteiros positivos},
avalie as seguintes expressões:

\begin{enumerate}[a. (\ )]
\item $\forall x : x\ {\tt\ é\ par} \land x\ {\tt\ é\ primo}$
\item $\exists x : x\ {\tt\ é\ par} \land x\ {\tt\ é\ primo}$
\item $\forall x : x\ {\tt\ é\ par} \lor x\ {\tt\ é\ primo}$
\item $\exists x : x\ {\tt\ é\ par} \lor x\ {\tt\ é\ primo}$
\item $\forall x : x\ {\tt\ é\ ímpar} \land x\ {\tt\ é\ primo}$
\item $\exists x : x\ {\tt\ é\ ímpar} \land x\ {\tt\ é\ primo}$
\item $\forall x : x\ {\tt\ é\ par} \lor x\ {\tt\ é\ primo}$
\item $\exists x : x\ {\tt\ é\ par} \lor x\ {\tt\ é\ primo}$
\end{enumerate}
\noindent Os números primos são divisíveis por si próprios e por $1$.\\
\noindent $\lor$ -- operador de disjunção (OU)\\
\noindent $\land$ -- operador de conjunção (E)\\

\ex{1,5} Dados os conjuntos $A=\{1\}$; $B=\{2\}$ e $C = \{\{1\}, 1\}$, classifique
...
\ex{2,5}~Para o dígrafo (grafo direcionado)
$$G=\{V,E\},$$
$$V=\{1,2,3,4,5,6,7,8,9\},$$
$$E=\{(1,2),(1,3),(2,4),(2,5),(2,7),(3,8),(4,1),(4,9)\},$$

\noindent determine:

\begin{enumerate}[a)]
\item O grau de cada vértice;
\item A listagem dos vértices que compõem o(s) ciclo(s) (se existir);
\item A sequência de vértices visitados usando o algoritmo de busca
  em profundidade (Depth-First Search--DFS);
\item A sequência de vértices visitados usando o algoritmo de busca
  em largura (Breadth-First Search--BFS).
\end{enumerate}
\noindent Realizar a busca caminhando para o vizinho mais próximo
de menor valor inteiro como identificação para eliminar ambiguidades
na escolha do próximo vértice a ser visitado.
